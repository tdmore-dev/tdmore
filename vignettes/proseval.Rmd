...
posthocFit <- dataTibble(object=m1, regimen=regimen, observed=db) %>% posthoc(se.fit=F, control=list(factr=1e14))
posthocFit %>% ungroup() %>%
...
a) discard subjects with missing data on day3 (target), as there is no point of reference in this case?
b) discard subjects with missing data on day1 or day2, as there is less data to calculate an accurate fit?
...
prosevalDb <- dataTibble(object=m1, regimen=regimen, observed=db) %>% proseval(control=list(factr=1e14))
```
...
target <- prosevalDb %>% 
  left_join(db %>% tidyr::nest(data=c(TIME, CONC)), by="ID") %>% # add original data
  tidyr::unnest(cols=c(data, ipred), names_sep=".") %>% # unnest, creating data.CONC and ipred.CONC columns
  group_by(ID, OBS) %>% 
