...
```{r population_prediction, eval=TRUE, results='hold', message=FALSE, warning=FALSE, fig.align='center', fig.height = 3, fig.width = 8}
data <- predict(
  object = tdmore,
...
```{r pred_ipred_predictions, eval=TRUE, results='hold', message=FALSE, warning=FALSE, fig.align='center', fig.height = 3, fig.width = 8}
...
```{r default_plot_function, eval=FALSE, results='hide'}
...
```{r find_dose_pred_ipred_predictions, eval=TRUE, results='hold', message=FALSE, warning=FALSE, fig.align='center', fig.height = 3, fig.width = 8}
ggplot(observed, aes(x=TIME, y=CONC)) + geom_point() +
  geom_line(aes(color="Individual"), data=predict(ipred, newdata=seq(0, 32, by=0.1))) +
  geom_ribbon(aes(fill="Individual", ymin=CONC.lower, ymax=CONC.upper), data=predict(ipred, newdata=data.frame(TIME=seq(0, 32, by=0.1), CONC=NA), se.fit=TRUE), alpha=0.25) +
  geom_line(aes(color="Population"), data=predict(pred, newdata=seq(0, 32, by=0.1))) +
  geom_ribbon(aes(fill="Population", ymin=CONC.lower, ymax=CONC.upper), data=predict(pred, newdata=data.frame(TIME=seq(0, 32, by=0.1), CONC=NA), se.fit=TRUE), alpha=0.1) +
  scale_color_manual(values=c("tomato1", "steelblue2")) +
