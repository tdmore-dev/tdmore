...
TDMore can be used to run simulations, based on the model defined in the previous step. For doing so, the regimen first needs to be specified. In the case of Meroponem, an 30-min injection is given into the central compartment every 8 hour. This can be written as follows:

```{r, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
regimen <- data.frame(
  TIME=seq(0, 3*8-1, by=8), # Every 8 hour and for 1 day, an injection is given
  AMT=rep(1000, 3),         # 1g is administered
  RATE=rep(1000, 3)/0.5     # 30-minute infusion (rate=dose/infusion time)
)
```
...
![Meropenem population plot](meropenem_simulation.png)
...
Now, let's assume blood samples have been collected for a subject X at different times. As an example, blood samples were collected at times 9h and 16h on the first day. This can be translated in TDMore as follows (note that the concentrations are purely fictive):
...
![Meropenem PRED vs IPRED plot](meropenem_pred_ipred.png)
...
![IPRED default plot](default_ipred_plot.png)
...
newRegimen <- data.frame(
  ...
  AMT=c(1000, 1000, 1000, NA),     # Adding an unknown dose on the secon day
...
The result of the findDose() routine is the following:
...
[1] 6.103516e-05
```
It tells us that 300 mg (approximately) is the recommended starting dose on the second day. The following code helps up verify this visually.

```{r, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
ipredRecommendedRegimen <- data.frame(
  TIME=c(0, 8, 16, 24),            # A fourth dose on the second day is added
  AMT=c(1000, 1000, 1000, 300),     # Adding the recommended on the second day
  RATE=c(1000, 1000, 1000, 1000)/0.5 # 30-minute infusion (rate=dose/infusion time)
)
...
![Resulting plot](find_dose_result_plot.png)
