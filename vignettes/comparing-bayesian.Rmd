...
Below, we simulate the standard treatment regimen of 100mcg twice-daily, with a loading dose of 500mcg.
...
result <- predict(population, newdata=seq(0, 20*24, by=4), 
  ...
  mutate(regimen="500ug + 100ug maint")
...
db <- predict(population, newdata=seq(7.5, 20*24, by=3*24),  #every 3 days
  ...
  rowwise() %>% mutate(fit = list( tdmorefit(m1, observed=observed.re, regimen=regimen, res = c(EKa=EKa, ECL=ECL, EV1=EV1, EV2=EV2, EQ=EQ)) ), object=list(fit$tdmore)) %>% ## Create the 'truth' using the true parameters used to simulate
  ungroup() %>% rename(ID=sample) %>% select(ID, fit, object)
``` 
...
optimize <- function(fit, regimen, truth) {
  ...
  regimen$AMT[i] <- plyr::round_any( regimen$AMT[i] * Fct, 20) #round to tablet size
  return( list(regimen=regimen) )
}

simulationResultNaive <- db %>%
  mutate(control=list(list(maxit=0)), se.fit=F) %>% #we do not need a fit; so use 'maxit=0' to not lose time with calculating a fit
  doseSimulation(optimize=optimize) %>%
  mutate(method="Naive")
...
simulationResultNaive %>% group_by(ID) %>% filter(row_number() == n()) %>%
  ...
  geom_tile(data=. %>% unnest(regimen) %>% filter(TIME!=0), aes(x=TIME, y=20, fill=factor(AMT))) +
  facet_wrap(~ID)
```
...
optimize <- function(fit, regimen, truth) {
  ...
  recommendation$regimen$AMT <- plyr::round_any(recommendation$regimen$AMT, 20) #round to tablet size
  return( list(regimen=recommendation$regimen) )
}

simulationResultEBE <- db %>%
  mutate(control=list(list(trace=0)), se.fit=F) %>% #do not calculate errors
  doseSimulation(optimize=optimize) %>%
  mutate(method="EBE")
...
result <- bind_rows(
  ...
  simulationResultEBE
) %>% ungroup() %>% mutate(method = factor(method)) %>%
  group_by(ID, method) %>% filter(row_number() == n())
result %>%
  ggplot(aes(x=TIME, y=CONC, color=method)) +
  facet_wrap(~ID) +
  annotate("rect", xmin=-Inf, xmax=Inf, ymin=8, ymax=12, fill="green", alpha=0.2) +
  geom_point(data=. %>% unnest(observed), aes(group=ID)) +
  geom_line(data=. %>% group_modify(function(x, ...){predict(x$fit[[1]], x$regimen[[1]], newdata=seq(0, 20*24))}), aes(group=method)) +
  geom_tile(data=. %>% unnest(regimen) %>% filter(TIME!=0), aes(x=TIME, y=21-as.numeric(method), fill=AMT), color=NA) +
  geom_text(aes(x=0, y=21-as.numeric(method), label=method), size=5) + 
  facet_wrap(~ID) +
  scale_fill_distiller(guide = "legend", breaks=seq(60, 200, by=20))
```

### Comparing both: stats comparison
```{r}
concentrations <- result %>% unnest(observed)
inTarget <- concentrations %>%
  mutate(INTARGET = between(CONC, 8, 12)) %>%
  group_by(method) %>% summarize(X = sum(INTARGET), N=n())
res <- prop.test(x=inTarget$X, n=inTarget$N )
res
```

Based on the stats comparison, 10 patients per arm may be enough (p=`r res$p.value`) to distinguish between naive dose optimization and computer dosing. We should run a simulation with 10,000 patients first to characterize the expected difference.
