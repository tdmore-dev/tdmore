...
#______________________________________________________________________________________________
#----                     TACROLIMUS ALGEBRAIC WITH IOV (KA only)                          ----
#______________________________________________________________________________________________
...
tacroTdm <- tdmore(tacroAlgebraic, omega = omega, res_var = list(errorModel("CONC", prop=0.149)), iov=c("EKa_IOV"))
...
observed <- data.frame(TIME=3*24, CONC=0.007)
ipred <- estimate(tacroTdm, regimen=regimen, covariates=covariates, observed=observed)

## Prediction at day 7 will be quite close to population, but prediction at day 4 should be more accurate
plot(ipred, newdata=data.frame(TIME=seq(0, 7*24, by=1), CONC=NA))
