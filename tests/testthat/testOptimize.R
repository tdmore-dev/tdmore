...
describe("optimize", {
  it("finds the right dose to hit a target", {
    ...
    rec <- optimize(ipred, regimen=myRegimen)

    newRegimen <- myRegimen
    newRegimen$AMT[2] <- 14.009638
    expect_equal(rec$regimen, newRegimen, tol=1e-2)
  })
  it("is equivalent to findDose for a single target", {
    ...
    rec2 <- findDose(ipred, regimen=myRegimen, doseRows=which(!myRegimen$FIX), target=tibble::tibble(TIME=myRegimen$TIME[2]+24, CONC=7.5))
    expect_equal(rec1, rec2)
  })
  it("Optimizes the whole treatment", {
    rec <- optimize(ipred)
    res <- predict(ipred, regimen=rec$regimen)
    expect_equal(res$CONC, rep(7.5, 5), tol=0.1) #equal to 7.5, more or less
